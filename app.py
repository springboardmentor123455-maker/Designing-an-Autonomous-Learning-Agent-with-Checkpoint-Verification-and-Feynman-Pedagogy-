...
st.set_page_config(page_title="AI Tutor Pro - HuggingFace", layout="wide")
...
@st.cache_resource
def load_huggingface_components(hf_token):
    if not hf_token:
    ...
    # Using Qwen 2.5 72B Instruct model
...
def generate_study_plan(llm, topic, doc_context):
    """
    Generates a 5-step curriculum using HuggingFace model.
    """
    source_instruction = ""
    ...
    if doc_context and len(doc_context) > 200:
        ...
        - If the document is about something completely different, return "NO".
...
def generate_lesson_content(llm, search_tool, topic, objective, doc_text):
    """
    Generates the lesson content using HuggingFace model.
    """
    # 1. SOURCE VALIDATION
    use_doc = False
    use_web = True
    ...
    if doc_text and len(doc_text) > 200:
        ...
        - "IRRELEVANT": The doc is about a completely different subject. (Use Web Only)
        ...
        print(f"Validation Status for '{topic}': {status}")
    ...
    3. IF the Context Materials contain information about unrelated topics, IGNORE THEM COMPLETELY.
    4. Focus ONLY on meeting the OBJECTIVE.
    5. Use simple, clear language suitable for students.
    """
...
def grade_answers(llm, topic, questions, answers):
    """
    Strictly grades answers using HuggingFace model.
    """
    # 1. PRE-PROCESS: fill empty answers
    cleaned_qa_pairs = []
    ...
    2. **Score 0** for factually incorrect or irrelevant information.
...
with st.sidebar:
    st.header("⚙️ Configuration - HuggingFace Edition")
    
    # 1. API Keys
    hf_input = st.text_input("HuggingFace API Token", type="password", help="Required for model access")
...
if not hf_input:
    st.info("Welcome! Please enter your HuggingFace API Token in the sidebar to begin.")
    st.stop()

# Initialize Models
llm, embeddings, search = load_huggingface_components(hf_input)

if not llm:
    st.error("Failed to initialize HuggingFace model. Please check your API token.")
    st.stop()
...
# VIEW: SETUP
if st.session_state.step == "setup":
    st.title("🎓 AI Tutor Pro - HuggingFace Edition")
