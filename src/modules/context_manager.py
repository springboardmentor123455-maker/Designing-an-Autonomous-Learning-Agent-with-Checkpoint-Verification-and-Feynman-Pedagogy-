...
class ContextManager:
    ...
    def gather_context(
        ...
        if user_notes and user_notes.strip():
            ...
            user_context = GatheredContext(
                source="user_notes",
                content=user_notes,
                gathered_at=datetime.now(),
                metadata={"length": len(user_notes)}
            )
            # Assign high relevance to user notes since they're user-provided
            user_context.relevance_score = 0.9
    ...
    def validate_context(
        ...
        # Score each context for relevance (skip if already scored)
        scored_contexts = []
        for context in contexts:
            if context.relevance_score is None:
